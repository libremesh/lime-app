...
class App extends Component {
  bannerOk = function () {
  	this.props.sendAction(this.props.meta.banner.onOK);
  }.bind(this)
  bannerCancel = function () {
  	this.props.sendAction(this.props.meta.banner.onCancel);
  }.bind(this)
  ...
  render ({ store, history }) {
  		...
  		return (
  			<div class={style.wraper}>
  				<Router history={history}>
  					<ChangeNode
  						path="/changeNode/:hostname"
  						change={this.props.changeNode}
  					/>
  					{plugins
  						.filter(plugin => plugin.page !== false)
  						.map(Component => (
  							<Component.page path={Component.name.toLowerCase()} />
  						))}
  				</Router>
  			</div>
  		);
  	...
  	return (
  		<div style={isBase(this.props.meta)}>
  			<Banner />
  			<Header
	hostname={this.props.meta.selectedHost}
  				goHome={this.props.goBase}
  				menuHidden={this.props.meta.menuHidden}
  			/>
