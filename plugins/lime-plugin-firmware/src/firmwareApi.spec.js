...
import { getUpgradeInfo, uploadFile } from "./firmwareApi";
...
describe("uploadFile", () => {
    ...
    it.each([
        ...
        "sends expected request to cgi-upload endpoint %s %s",
        async (userFileName, uploadedFilename) => {
            api.sid.mockImplementation(
                () => "4261b19d65fac6be2552e10d3a351b5c"
            );
    ...
    it.each([
        ...
        "resolves to destination path when the upload finished correctly",
        async (userFileName, uploadedFilename) => {
            const mockResponse = {
                size: 5374131,
                checksum: "ef2949e0070830d49e3930a00f9d1794",
                sha256sum:
                    "3cf8244acc06dd0d852bb2348e0da4f610dda6f0d3eed5807d4fe12570465a24",
            };
            api.sid.mockImplementation(
                () => "4261b19d65fac6be2552e10d3a351b5c"
            );
            ...
            xhrMock.post("/cgi-bin/cgi-upload", (_req, res) =>
                res.status(200).body(JSON.stringify(mockResponse))
            );
