...
import { getAssocList, getMeshIfaces } from "./src/alignApi";
...
describe("align page", () => {
    ...
    beforeEach(() => {
        getMeshIfaces.mockImplementation(async () => [
            "wlan1-mesh",
            "wlan2-mesh",
        ]);
    ...
    it("shows one tab for each mesh iface with its radio name", async () => {
        render(<AlignPage />);
        expect(
            await screen.findByRole("tab", { name: /radio 1/i })
        ).toBeInTheDocument();
        expect(
            await screen.findByRole("tab", { name: /radio 2/i })
        ).toBeInTheDocument();
    });
    ...
    it("shows a message when there are not neighbors in one radio", async () => {
        ...
        expect(
            await screen.findByText(
                /This radio is not associated with other nodes/i
            )
        ).toBeInTheDocument();
    ...
    it.skip("orders the neighbors by signal strength but avoid jumpy reordering", async () => {
        getAssocList
        ...
        render(<AlignPage />);
        expect(await findNeighbors()).toEqual([
            "mc-rocio",
            "mc-martinez",
            "mc-mile",
        ]);
        expect(getAssocList).toHaveBeenCalledTimes(1);
        act(() => {
            jest.advanceTimersByTime(2 * 1000);
        });
        await flushPromises();
        expect(getAssocList).toHaveBeenCalledTimes(2);
        expect(await findNeighbors()).toEqual([
            "mc-rocio",
            "mc-martinez",
            "mc-mile",
        ...
        await flushPromises();
        expect(getAssocList).toHaveBeenCalledTimes(3);
        expect(await findNeighbors()).toEqual([
            "mc-rocio",
            "mc-mile",
            "mc-martinez",
    ...
    it("shows nodes with big inactive time as not associated anymore", async () => {
        getAssocList.mockImplementationOnce(async () => [
        ...
        expect(
            await screen.findByText(/last packet: 4 seconds ago/i)
        ).toBeInTheDocument();
    });
});
