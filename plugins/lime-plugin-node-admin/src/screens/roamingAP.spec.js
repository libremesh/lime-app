...
import { getAPsData, setupRoamingAP } from "../nodeAdminApi";
...
describe("roaming ap config", () => {
    ...
    it("shows a text explaining the config", async () => {
        ...
        expect(
            await screen.findByText('Opens the "QuintanaLibre" AP in this node')
        ).toBeVisible();
        expect(
    ...
    it("shows the default value for the community, case disabled", async () => {
        ...
        expect(
            await screen.findByText(
                "It is disabled by default in QuintanaLibre"
            )
        ).toBeVisible();
    });
    ...
    it("shows the default value for the community case enabled", async () => {
        ...
        render(<RoamingAPPage />);
        expect(
            await screen.findByText("It is enabled by default in QuintanaLibre")
        ).toBeVisible();
    });
    ...
    it("calls api endpoint for disabling ap when switched off", async () => {
        ...
        await waitForExpect(() => {
            expect(setupRoamingAP).toHaveBeenCalledWith({
                enabled: false,
            });
        });
